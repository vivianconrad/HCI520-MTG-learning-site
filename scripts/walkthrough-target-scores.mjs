/**
 * Full app walkthrough with configurable pre/post test scores.
 * Usage: npm run dev (separate terminal), then:
 *   node scripts/walkthrough-target-scores.mjs
 *   PRETEST_TARGET=2 POSTTEST_TARGET=9 node scripts/walkthrough-target-scores.mjs
 */
...
async function main() {
  ...
  await page.getByText(/Saving your lesson progress/i).waitFor({ state: 'hidden', timeout: 60_000 }).catch(() => {})
  await page.waitForFunction(
    () => {
      const disabled = [...document.querySelectorAll('.lesson-complete__readiness-btn')].some(
        (btn) => !btn.disabled
      )
      return disabled
    },
  ...
  await page.getByRole('button', { name: /Somewhat ready/i }).click()
  await page.getByText(/Saving your response/i).waitFor({ state: 'hidden', timeout: 30_000 }).catch(() => {})
