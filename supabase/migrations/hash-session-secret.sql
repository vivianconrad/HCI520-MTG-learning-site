...
create or replace function public.hash_session_secret(p_secret text)
returns text
language sql
immutable
...
begin
  if exists (select 1 from public.schema_migrations_applied where name = 'hash-session-secret') then
    ...
    set session_secret = public.hash_session_secret(session_secret);
...
begin
  ...
  insert into public.participants (
    ...
    p_session_id,
    p_session_id,
    public.hash_session_secret(p_session_secret),
...
revoke all on function public.hash_session_secret(text) from public;
-- Not granted to anon: only security-definer RPCs call this internally.
