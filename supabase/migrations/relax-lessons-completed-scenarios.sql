-- Lesson 4 practice scenarios are optional in the app; participants may reach
-- lessons_completed with scenarios_attempted < 11. Drop the old trigger guard
-- that blocked those saves (and blocked posttest when lessons_completed never persisted).
--
-- Run in Supabase SQL Editor, then verify: node scripts/debug-save-flow.mjs
